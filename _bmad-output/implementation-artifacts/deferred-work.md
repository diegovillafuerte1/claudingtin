...
- source_spec: `_bmad-output/implementation-artifacts/spec-2-2-curated-opener-set-and-per-match-selection.md`
  summary: A malformed `backend/openers.txt` fails closed only via a `panic` thrown from inside `hub.Run`'s goroutine at startup (it calls `Openers()` before its command loop). There is no eager validation in `backend/cmd/serve/main.go` producing a clean fatal log line before the hub goroutine starts.
  evidence: blind-hunter + verification-gap. Behaviour is spec-sanctioned (fail-closed at boot) and the process does crash immediately, but the failure surfaces as a raw goroutine panic rather than a structured "bad opener set" error. Fix candidate: call `backend.Openers()` (or a validate-only variant) in `main()` and `log.Fatal` with the parse error before `go h.Run(...)`.

- source_spec: none
  summary: The `test (windows-latest)` CI job is flaky in the `companion` module — `TestWatcherRecoversFromRotation` (`companion/internal/transcript`, fails on `rename` of an open `session.jsonl`: "The process cannot access the file because it is being used by another process") and `TestResolveExistingWaitsForWinner` (`companion/internal/identity`, ~2.2s timing-sensitive) each failed once across two consecutive runs of the same green commit (PR #15).
  evidence: Neither test is in Story 2.2's change surface (backend/proto only); `backend` tests pass on Windows every run; a prior main merge (PR #13) also shows a Windows CI failure. Root cause is Windows filesystem/timing sensitivity: the watcher likely holds an open handle when the rotation test renames, and the identity election test's wait window is too tight for Windows runners. Fix candidates: close/reopen the watcher handle (or retry the rename) in the test; widen or make deterministic the identity-winner wait.

- source_spec: `_bmad-output/implementation-artifacts/spec-2-4-in-memory-message-relay.md`
  summary: `run.loop` can deadlock on `chat.Println` if it processes a buffered `chatIntents` (block/report/leave) or `chatSends` (send-failure notice) item after the Bubble Tea chat program's `Run()` has already returned. `*tea.Program.Println` is a bare blocking send on an unbuffered internal `p.msgs` with no `ctx.Done()` escape; once `Run()` returns nothing drains it. The `<-chatDone` teardown arm and the intent/send arms are both select-ready in that window, so ~50% of the time `loop` picks the wrong one and hangs forever.
  evidence: edge-case-hunter + blind-hunter. Pre-existing from Story 2.3 (the three block/report/leave `chat.Println` sites); Story 2.4 adds one more (`"companion: a chat line could not be sent"`). Reachable on every keyboard `leave`: the model calls `notify(IntentLeave)` (non-blocking send into `chatIntents`) and then returns `tea.Quit` in the same Update, so an IntentLeave is always buffered when `chatDone` closes. Usually `loop` drains it before `Run()` finishes tearing down, so the hang is a tail race. `chat.Send` (the new inbound `PeerMsg` forward) is NOT affected — `*tea.Program.Send` has a `<-p.ctx.Done()` branch and returns immediately after `Run()` exits. Fix candidate: track a `chatRunning` flag cleared when the `Run()` goroutine exits (or prefer the `<-chatDone` case), and gate every `chat.Println`/`chat.Send` on it rather than on `chat != nil`.

- source_spec: `_bmad-output/implementation-artifacts/spec-epic-2-retro-item-17-windowed-placement-fallback.md`
  summary: The Windows new-window fallback uses `cmd /c start` and is unverified on a real windows-latest host — cmd.exe may collapse the trailing empty argv, making the companion exit 2.
  evidence: No Windows CI leg exercises the window path (TestRunWindowTool skips on windows; compiled-launcher e2e tests set CLAUDINGTIN_NO_WINDOW=1). Needs a manual check that `cmd /c start "" "<bin>" "<transcript>" "" ""` opens a visible console and the companion receives [transcript,"",""] intact; if not, switch to `conhost` (frozen-intent change).

- source_spec: `_bmad-output/implementation-artifacts/spec-epic-2-retro-item-17-windowed-placement-fallback.md`
  summary: Linux terminal `-e` / `--` flag conventions vary; a first-present emulator with incompatible flags falls straight to the inert detached spawn (no retry across the probe list, per the F10 boundary).
  evidence: `x-terminal-emulator -e` (Debian alternatives) and pre-3.x `gnome-terminal` (wanted `-e`, not `--`) are the fragile entries. windowterm.go picks the first on PATH and does not try the next on failure. Consider a post-open liveness confirmation or a bounded retry.

- source_spec: `_bmad-output/implementation-artifacts/spec-epic-2-retro-item-10-panemanager-extraction.md`
  summary: `TestIntentRacingChatProgramExitDoesNotWedgeLoop` (`companion/internal/run`, the Epic 2 retro F6 guard) is a pre-existing timing flake — under `go test -race` it fails ~2/6 full-suite runs locally with "no chat program constructed after matched" (run_test.go:1484), yet passes 5/5 in isolation. Verified to flake at the same rate on `origin/main` with none of the item-10 changes applied, so it is not caused by the paneManager extraction.
  evidence: Reproduced on a clean baseline checkout (stashed the item-10 diff): 1/4 then 2/6 full-suite failures. The test buffers `IntentBlock`/`IntentLeave`, quits the fake chat program, waits on `f.exited`, then sends a second `matched` and expects `waitChat` (2s) to see a new program. `f.exited` is closed by the fake's `Run()` a scheduling hop before `pane.start`'s goroutine closes `chatDone`, so under `-race` load `loop` can still be draining the first program's teardown (a bounded `time.After(chatQuitGrace)` wait in `pane.teardown`, 100ms in tests) when the second `matched` lands, occasionally pushing `waitChat` past its deadline. Same class as the `TestResolveExistingWaitsForWinner` / `TestWatcherRecoversFromRotation` entries above. Fix candidate: in the test, wait for `loop` to quiesce after `f.exited` (e.g. poll until a status-line repaint or a `chatDone`-driven teardown is observed) before sending the second `matched`, or widen `waitChat`'s deadline for this case.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-1-unified-session_ended-on-every-backend-side-end.md`
  summary: A connection that already received its `session_ended` for a match and then sits idle/queued (no new `matched`) gets a bare normal socket close — no final `session_ended` frame — when it is later taken over by a second `hello` for the same key (or on server shutdown). The new per-connection `sentEnd` latch in `serveConn` is cleared only by `proto.Matched`, so the `Evict` case's `if !sentEnd` guard suppresses the frame.
  evidence: blind-hunter + edge-case-hunter + verification-gap all flagged the `sentEnd` latch's one-way behaviour. Consistent with the spec's "at most one session_ended per matched session" invariant, and unreachable by today's bundled companion (it exits on the first `session_ended`, run.go:330), so no current consumer is affected. Becomes reachable once Story 3.5 (silent re-enqueue) keeps a socket alive across matches, or with any non-companion client that keys session state off `session_ended` rather than the socket close. The common takeover path (a never-matched connection being displaced) is unchanged and still covered by TestTakeover_PriorConnEndedNewConnActive. Revisit when Story 3.5 lands: decide whether an idle displaced connection should still get a terminal `session_ended`, or whether the normal close is the contract.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-2-their-claude-came-back-presentation-and-no-alarm-language.md`
  summary: AC1's second end path — "local model-return with persist off" ends the chat and shows the "their Claude came back" state — is not built. Story 3.2 covers only the backend `session_ended`-received path; the model returning does not end a chat.
  evidence: The persist toggle is Story 3.6 and ships default-ON (a returning model does NOT auto-end a chat), so wiring an auto-end-on-model-return path now would ship a persist-OFF default that contradicts epic-3-context.md. The "sends `leave`" half of that path is Story 3.3. Both prerequisites land later in Epic 3. When 3.6 adds the toggle and 3.3 adds `leave`, route the persist-off model-return through the same `run.loop` presentation the `client.SessionEnded()` arm uses (spinner if `desired == stateReady`, `statusline.PhaseClaudeBack` if `stateBusy`). Confirmed with the user at planning time (defer, not wire-dead-gated).

- source_spec: `_bmad-output/implementation-artifacts/spec-3-2-their-claude-came-back-presentation-and-no-alarm-language.md`
  summary: On a connection takeover the companion can briefly launch the searching spinner (or paint `PhaseClaudeBack`) and then tear it down microseconds later. The `client.SessionEnded()` arm surfaces the in-band `session_ended` frame before wsclient detects the socket close, so `run.loop` routes the pane, then the `clientDone`/`ResultSessionEnded` arm runs `pm.stopAll()` and exits. Cosmetic flash, takeover-only (a second companion for the same account key — rare).
  evidence: edge-case-hunter + blind-hunter. Pre-change, `session_ended` was terminal in wsclient so `run.loop` only ever saw the `clientDone` teardown — no in-band routing, no flash. Now the frame and the close arrive on two paths with no ordering guarantee. A clean fix needs takeover-awareness in `loop` (e.g. a short-lived "close pending" latch, or Story 3.4's grace window making takeover a distinct signal). Low severity; revisit with Story 3.4.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-2-their-claude-came-back-presentation-and-no-alarm-language.md`
  summary: A peer's final `chat_msg` can be dropped at chat end. `client.ChatMsgs()` (buffer 64) and `client.SessionEnded()` (buffer 1) are separate `run.loop` select cases; when both are ready Go picks at random, so the `SessionEnded()` arm can call `pm.stopChat(false)` before the last buffered peer line is forwarded to the surface.
  evidence: blind-hunter + edge-case-hunter. The same render-order race existed pre-change via the old `clientDone`/`ResultSessionEnded` teardown (it too raced `ChatMsgs()`), but it ended in process exit so a lost trailing line was moot; now the companion keeps running and the drop is user-visible. Story 3.2's frozen scope explicitly excludes late-peer-`chat_msg` handling ("Story 3.3"). Story 3.3 ("Late peer `chat_msg` after leaving is not shown") should decide whether a drain-before-teardown or an explicit "show the last line, then end" is wanted, and for the `session_ended` path too, not only local `leave`.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-2-their-claude-came-back-presentation-and-no-alarm-language.md`
  summary: `session_ended` racing `matched` for the same session can strand a chat pane. If `run.loop` services the `client.SessionEnded()` arm while a `matched` for that same (now-ended) session is still buffered on `client.Matched()`, the `SessionEnded()` arm's `pm.stopChat(false)` is a no-op (no chat up yet), then the `Matched()` arm calls `pm.launchChat` and opens a surface that no further `session_ended` will ever close.
  evidence: blind-hunter. Requires the backend to emit `matched` then `session_ended` for one session close together AND `run.loop` to pick `SessionEnded()` first from the select — narrow. Pre-change this ended in process exit (moot). Belongs with the pairing-lifecycle hardening in Stories 3.4 / 3.5; candidate fix: have the `SessionEnded()` arm also drain/ignore a stale buffered `matched`, or gate `launchChat` on a recent-end guard.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-2-their-claude-came-back-presentation-and-no-alarm-language.md`
  summary: wsclient has no half-open-socket detection (no read deadline, no keepalive ping). With `session_ended` no longer terminal, a takeover whose close never reaches the client (half-open TCP) leaves `serve` blocked in `conn.Read` forever — no `serveSessionEnded`, no reconnect.
  evidence: edge-case-hunter. Pre-existing and broader: the same hang is reachable on any idle socket after `matched` today. This change adds one more entry path (`session_ended` then silence). Fix is a wsclient-wide concern — a `conn.Read` deadline reset per frame, or periodic `conn.Ping` — sized for its own story, likely alongside Story 3.4 (reconnect grace) which already touches connection liveness.

- source_spec: `_bmad-output/implementation-artifacts/spec-3-2-their-claude-came-back-presentation-and-no-alarm-language.md`
  summary: The takeover heuristic (`session_ended` + socket close within `sessionEndedCloseGrace`) is purely time-based. Also checking `websocket.CloseStatus(err) == websocket.StatusNormalClosure` would separate a graceful server close (takeover / shutdown) from an abrupt network drop and shrink the false-positive window that scales with the 2 s bound.
  evidence: edge-case-hunter + blind-hunter. The spec deliberately chose a time-only bound for simplicity and the user approved it; adding a close-code check is a hardening with its own test surface (what `coder/websocket` returns for each drop kind), not a trivial patch. Revisit if field data shows companions exiting on ordinary blips right after a peer leaves.
