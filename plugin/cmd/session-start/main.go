...
// companion one of three ways:
//
//   - inside tmux ($TMUX set): an adjacent tmux split-window pane beside the
//     Claude session that does not take focus;
//   - no tmux (or the split failed): the companion spawned detached with
//     (transcript-path, "", "") without waiting, plus exactly one stdout line
//     telling the user how to open a live view themselves;
//   - any precondition unmet (opt-out, malformed input, missing binary,
//     session already launched, …): a silent no-op.
//
// It is fail-open: every path — including a tmux failure that falls back to the
// detached spawn, a spawn failure, and a panic in the core — ends at the single
// deferred os.Exit(0). It never exits non-zero (a non-zero SessionStart hook,
// in particular exit 2, blocks the session) and never blocks on the child. The
// only thing ever written to stdout is that single no-tmux hint line, and only
// once the detached spawn has succeeded. All policy lives in launch.go's pure
// core; this file only wires the real stdin / env / temp-dir / stdout / spawn /
// tmux seams and maps the result to the exit.
package main
